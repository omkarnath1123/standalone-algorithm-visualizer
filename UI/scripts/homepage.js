// var container = d3.select("#page").append("svg");
window.d3 = require("d3v3");
...
map.canvas.nodes = d3.range(100).map(function(d, i) {
  return {
...
map.svg.nodes = d3.range(100).map(function(d, i) {
  return {
    x: (Math.random() * map.width) / 2,
    y: (Math.random() * map.height) / 2,
    r: Math.random() * 10 + 3
